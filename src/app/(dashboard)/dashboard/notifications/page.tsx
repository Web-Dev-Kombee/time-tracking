...
export default function NotificationsPage() {
  ...
  const getFilteredNotifications = () => {
    ...
    switch (selectedTab) {
      ...
      case "timer":
        return data.notifications.filter(
          (notification: Notification) => notification.type === "running_timer"
        );
      case "payment":
        return data.notifications.filter(
          (notification: Notification) => notification.type === "payment_received"
        );
      default:
        ...
        <div className="flex justify-between items-center mb-4">
          <TabsList>
            ...
            <TabsTrigger value="invoice">
              Invoices
              {data?.counts && data.counts.overdue + data.counts.upcoming > 0 && (
                <span className="ml-2 bg-blue-100 text-blue-800 text-xs rounded-full px-2 py-0.5">
            ...
            <TabsTrigger value="timer">
              Timers
              {data?.counts && data.counts.running > 0 && (
                <span className="ml-2 bg-blue-100 text-blue-800 text-xs rounded-full px-2 py-0.5">
                  {data.counts.running}
                </span>
              )}
            ...
            <TabsTrigger value="payment">
              Payments
              {data?.counts && data.counts.payments > 0 && (
                <span className="ml-2 bg-blue-100 text-blue-800 text-xs rounded-full px-2 py-0.5">
