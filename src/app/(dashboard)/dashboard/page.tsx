import { CurrentTimeEntry } from "@/components/dashboard/CurrentTimeEntry";
...
export default async function DashboardPage() {
  ...
  // Fetch current time entry if exists
  const session = await getServerSession(authOptions);
  let currentTimeEntry = null;

  if (session?.user) {
    currentTimeEntry = await prisma.timeEntry.findFirst({
      where: {
  ...
  return (
    <div className="space-y-6">
      ...
      {currentTimeEntry && (
        <CurrentTimeEntry
          initialEntryId={currentTimeEntry.id}
          initialStartTime={currentTimeEntry.startTime.toISOString()}
          initialDescription={currentTimeEntry.description || undefined}
          initialProjectName={currentTimeEntry.project.name}
          initialClientName={currentTimeEntry.project.client.name}
        />
      )}
